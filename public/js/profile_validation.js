...
function inputCheck(input, item_name, name, min_length, max_length, errors) {
    ...
    if (typeof input !== "string") {
        ...
        errors[item_name] = `Error: Invalid input for ${name}. No HTML elements are allowed.`;
    } else if(min_length){
        if (input.length < min_length) {
            errors[item_name] = `Error: ${name} must be at least ${min_length} characters long.`;
        }
    } else if(max_length){
        if (input.length > max_length) {
            errors[item_name] = `Error: ${name} must be at most ${max_length} characters long.`;
        }
    }
}
...
$('#editProfileModal').submit((event) => {
    ...
    if(Object.keys(errors).length != 0){
        event.preventDefault();
        if(errors.firstName){firstNameError.html(errors.firstName)};
        if(errors.lastName){lastNameError.html(errors.lastName)};
        if(errors.email){emailError.html(errors.email)};
        if(errors.city){cityError.html(errors.city)};
        if(errors.state){stateError.html(errors.state)};
        if(errors.desiredPosition){desiredPositionError.html(errors.desiredPosition)};
        if(errors.dreamJob){dreamJobError.html(errors.dreamJob)};
    }
...
$('#changePasswordModal').submit((event) => {
    ...
    validatePassword(confirmNewPassword, 'confirmNewPassword', 'Confirm new password', errors);

    //Ensure that new password and confirm new password match
    if(newPassword !== confirmNewPassword){
        errors.confirmNewPassword = 'Error: New password and Confirm New Password do not match. Please try again.';
    }
